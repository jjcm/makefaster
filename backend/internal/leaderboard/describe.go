...
func init() {
	for name, description := range map[string]string{
		...
		"Enable Gzip Compression":          "Compress text responses (HTML, JS, CSS, JSON) with gzip so first-load transfer is smaller.",
		"Enable Brotli Compression":        "Prefer brotli for text responses when the client accepts it, falling back to gzip.",
		"Precompress Static Assets":        "Pre-generate gzip/brotli siblings for static text assets and serve them to clients that accept those encodings.",
