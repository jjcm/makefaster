...
// RerankCategories puts the biggest average improvement first (deltas are
// negative, so ascending pct), breaks ties on count, then name for stability,
// and rewrites every rank to its 1-based position.
...
//   - below threshold -> the improvement is novel, so a new category is created
//     on the leaderboard, seeded from the submission.
//
...
func Categorize(improvements []Improvement, categories []Category, embedder embedding.Embedder, threshold float64) ([]Category, []CategorizeResult) {
	...
	for _, improvement := range improvements {
		texts = append(texts, EmbeddingText(improvement.Name, improvement.Description))
	}
