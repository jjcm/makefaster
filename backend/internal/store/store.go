...
func (s *Store) Sites(ctx context.Context) ([]leaderboard.SiteRow, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT name, url, pr_url, favicon, lcp_before, lcp_raw, lcp_delta, tti_before, tti_raw, tti_delta, mode, tests, measured_at
		FROM sites
		ORDER BY id`)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.Name, &row.URL, &row.PRURL, &row.Favicon, &row.LCPBefore, &row.LCPRaw, &row.LCPDelta,
...
func (s *Store) UpsertSite(ctx context.Context, submission leaderboard.SiteSubmission, now time.Time) (leaderboard.SiteRow, bool, error) {
	...
	).Scan(&current.Name, &current.URL, &current.PRURL, &current.Favicon, &current.LCPBefore, &current.LCPRaw, &current.LCPDelta,
	...
	if created {
		_, err = tx.ExecContext(ctx, `
			INSERT INTO sites (name, url, pr_url, favicon, lcp_before, lcp_raw, lcp_delta, tti_before, tti_raw, tti_delta, mode, tests, measured_at)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		_, err = tx.ExecContext(ctx, `
			UPDATE sites SET name = ?, pr_url = ?, favicon = ?, lcp_before = ?, lcp_raw = ?, lcp_delta = ?,
				tti_before = ?, tti_raw = ?, tti_delta = ?, tests = ?, measured_at = ?
			WHERE url = ? AND mode = ?`,
			row.Name, row.PRURL, row.Favicon, row.LCPBefore, row.LCPRaw, row.LCPDelta,
...
func (s *Store) seedSites(ctx context.Context, seedDir string) error {
	...
	for _, row := range rows {
		...
		if _, err := statement.ExecContext(ctx, row.Name, row.URL, row.PRURL, row.Favicon, row.LCPBefore, row.LCPRaw,
